...
/***data***/
struct editorConfig {
    int cx, cy;                  // Cursor x (column) and y (row) position in the editor
    int screenrows;              // Number of rows visible in the terminal window
    int screencols;              // Number of columns visible in the terminal window
...
void editorDrawRows(struct abuf *ab) { // Draws each row of the editor to the append buffer
    ...
    for(y = 0; y < E.screenrows; y++) {
        if(y == E.screenrows / 3) { // Displays welcome message at one-third down the screen
...
void editorRefreshScreen() { // Redraws the entire editor screen
    ...
    snprintf(buf, sizeof(buf), "\x1b[%d;%dH", E.cy + 1, E.cx + 1); // Sets cursor position
